...
<!---		<cfset thisResult = '{"result":"#passFail#","reason":"#reason#","customer_list":"#customersList#","grade_list":"#gradesList#","style_list":"#stylesList#","machine_list":"#machinesList#"}'>--->
		...
		<cfcatch>
		<cfmail to="tfafard@carolinacontainer.com" from="tfafard@carolinacontainer.com" subject="ERROR: CustomerUtil Typeahead Fail" type="HTML">
			<cfdump var="#cfcatch#">
		</cfmail>
		</cfcatch>
		</cftry>
	...
	<cffunction name="doSearch" access="remote" output="yes">
		...
		<cftransaction isolation="READ_UNCOMMITTED">
			<cfquery name="getData" datasource="Corporate">
				SELECT
					DLV_DATE,
					LEFT(REPLACE(csname,',',' '),70) as csname,
					Plant,
					Drno,
				...
				<cfif arguments.customer neq "">
					AND csname = <cfqueryparam value="#arguments.customer#" cfsqltype="CF_SQL_VARCHAR">
				</cfif> 
		...
		<cfif getData.recordcount>
			...
			<cfloop query="getData">
				...
				<cfset tableString = tableString & "<tr class='appendedRow' height='40px'><td class='bBorder' width='13.44%' data-sort='" & DateTimeFormat(getData.dlv_date,"yyyymmddHHnnss") & "'><div align='right'>" & DateFormat(getData.dlv_date,"mm/dd/yyyy") & "</div></td><td class='bBorder' width='13.44%'><div align='right'>" & getData.plant & "</div></td><td class='bBorder' width='17.36%'><div align='right' class='mainCol'>" & UCASE(UCASE(REReplaceNoCase(REReplaceNoCase( getData.csname ,"[^a-z0-9]" , " " , "all" ), chr(34), "", "all"))) & "</div></td><td class='bBorder' width='13.44%'><div align='right'>" & getData.CARRIER_TRUCK_CD & "<br><span style='font-size:8pt;color:rgba(0,0,0,0.4)'>( " & getData.driver & " )</span></div></td><td class='bBorder' width='13.44%'><div align='center'>" & getData.tripno & "</div></td><td class='bBorder' width='13.44%'><div align='left'>" & getData.drno & "</div></td><td class='bBorder' width='13.44%'><div align='left'>" & lsNumberFormat(getData.MSF_CFLUTE_EQUIV,".999") & "</div></td></tr>">
					
				
				
				<cfif carrierTrip neq getData.carrier_truck_cd & "" & getData.tripno>
					<cfset totalTrips += 1>
				</cfif>	
				
				<cfset carrierTrip = getData.carrier_truck_cd & "" & getData.tripno>	
		...
		<cfset thisResult = thisResult & '"result":"#passFail#","reason":"#reason#","plant":"#arguments.plant#","start_date":"#arguments.startDate#","end_date":"#arguments.endDate#","customer":"#arguments.customer#","total_trips":"#totalTrips#","total_shipped_msf":"#totalShippedMSF#","tableString":"#tableString#"}'>
...
--->
		...
		<cfcatch>
			<cfmail to="tfafard@carolinacontainer.com" from="tfafard@carolinacontainer.com" subject="ERROR: Customer Utilization Search" type="HTML">
				<cfdump var="#cfcatch#">
			</cfmail>
		</cfcatch>
		</cftry>
	...
	<cffunction name="getChartData" access="remote" output="yes">
		<cfargument name="plant">
		<cfargument name="customer" default="">
		<cfargument name="startDate">
		<cfargument name="endDate">
		...
		<cfswitch expression="#arguments.chartType#">
			...
			<cfcase value="CustTotalMSF">
				<cfquery name="getData" datasource="Corporate">
					SELECT
						...
						LEFT(REPLACE(csname,',',' '),70) as x_data,
			...
			<cfcase value="CustNumberTrips">
				<cfquery name="getData" datasource="Corporate">
					SELECT
						...
						LEFT(REPLACE(csname,',',' '),70) as x_data,
    					COUNT(distinct Trip_Row_ID) as y_data
					FROM
						IDR_Summary_Customer
					WHERE DLV_DATE >= <cfqueryparam value="#arguments.startDate#" cfsqltype="CF_SQL_VARCHAR">
					AND DLV_DATE <= <cfqueryparam value="#arguments.endDate#" cfsqltype="CF_SQL_VARCHAR">
					...
					ORDER BY
						LEFT(REPLACE(csname,',',' '),70),
						COUNT(distinct trip_row_id) desc
				</cfquery>
			...
			<cfcase value="CustAvgMSFTrip">
				<cfquery name="getData" datasource="Corporate">
					select a.Plant, a.x_data, AVG(a.y_data) as y_data from (
						SELECT
							...
							LEFT(REPLACE(csname,',',' '),70) as x_data,
							(sum(MSF_CFlute_Equiv) / count(distinct Trip_Row_ID)) as y_data
						FROM
							IDR_Summary_Customer
						WHERE DLV_DATE >= <cfqueryparam value="#arguments.startDate#" cfsqltype="CF_SQL_VARCHAR">
						AND DLV_DATE <= <cfqueryparam value="#arguments.endDate#" cfsqltype="CF_SQL_VARCHAR">
				...
				<cfset yDataFormat = ".99">
				<cfset chartName = "Average MSF / Trip: By Customer">
			</cfcase>
		...
		<cfif getData.recordcount>
			...
			<cfset thisCust = arguments.customer>
		...
		<cfset thisResult = thisResult & '"result":"#passFail#","reason":"#reason#","this_plant":"#thisPlant#","this_cust":"#thisCust#","chart_name":"#chartName#","x_arr":#xArr#,"data_obj":#dataObj#}'>
