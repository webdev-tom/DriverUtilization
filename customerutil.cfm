...
<head>
<meta charset="UTF-8">
<title>Customer Utilization Demo | tomfafard.com</title>
				...
				<li id="nav-right" style="position: static">
					...
					<div id="reportTitleDiv" class="btn-group">
						<h1 id="reportTitle" class="noselect dropdown-toggle hoverspinner" data-toggle="dropdown">
							 <i id="titleIcon" class="fas fa-users" style="display: none"></i> Customer Utilization</h1>
...
--->
				...
				<div id="reportFilterFormDiv" class="form-group">
					<form action="customerutil.cfm" method="put" name="reportFilterForm" id="reportFilterForm">
						
						<h4 id="searchHeader" class="noselect">search customer:</h4>

					
						<div id="customerDiv" class="inputStyle" style="width: 40% !important">
							<div class="typeahead__container">
								<div class="typeahead__field">
									<div class="typeahead__query">
										<input type="search" id="customerInput" class="form-control" name="customer" placeholder="name" autocomplete="off" spellcheck="false">
									</div>
								</div>
							</div>
						</div>
						
						<div id="plantDiv" class="selectStyle" style="width: 20% !important">
							<h4 id="rangeHeader" class="noselect" style="position: absolute;top: 0px">select plant:</h4>
...
--->
					...
					<span id="customerInfo">( leave blank for all customers )</span>
				</div>
			</div>
		</div> <!--- inputRow --->
			...
			<div class="col-sm-12" >
				...
				<div id="reportTabs" class="container" style="margin-top: 20px;display: none">	
					<ul  class="nav nav-tabs nav-justified">
						<li class="active">
							<a  href="##1a" data-toggle="tab"><span id="tabhdr1" class="tabtitle">Results: Unique DRs by Customer</span></a>
						</li>
						<li>
					...
					<div class="tab-content clearfix">
						...
						<div class="tab-pane" id="2a" style="padding-top: 10px">
							...
							<div id="chartSelect">
								<div class="btn-group">
									<button type="button" class="form-control btn btn-default dropdown-toggle" data-toggle="dropdown">
										<span id="selectedChart">MSF Shipped: by Customer</span> <span class="caret"></span>
									</button>
									<ul id="chartMenu" class="dropdown-menu" role="menu">
										
										<li style="border-top: 1px solid ##ddd"><a href="##" data-showdiv="CustTotalMSFDiv">MSF Shipped: by Customer</a></li>
										<li><a href="##" data-showdiv="CustNumberTripsDiv">Number Trips: by Customer</a></li>
										<li><a href="##" data-showdiv="CustAvgMSFTripDiv">Average MSF / Trip: by Customer</a></li>
...
--->
								...
								<div id="CustTotalMSFDiv" class="chartDiv activeChart"></div>
	

								<div id="CustNumberTripsDiv" class="chartDiv"></div>
							
							
								<div id="CustAvgMSFTripDiv" class="chartDiv"></div>
	  ...
	  <div class="modal-dialog" role="document" style="width: 300px !important">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			...
			<form action="customerutil.cfm" method="put" name="advancedFilterForm" id="advancedFilterForm">
...
//		var firstRunTicket = 1;
		...
		$.ajax({
				type: "get",
				url: "customerutil_CFC.cfc?method=getTypeaheadData",
				dataType: "text",
				data: {
					ready: areweready
				},
				...
				success: function( data ){
					...
					if (obj.result == 'pass') {
						...
						var customer_array = obj.customer_list.split("*");
						...
						console.log("Typeahead failed to initiate!");
		...
		var CHART_VARS = ["CustTotalMSFChart","CustNumberTripsChart","CustAvgMSFTripChart"];
		var thisSelectedDiv;
		var lastSelectedDiv = "CustTotalMSFDiv";
		$('.chartDiv').hide();
		...
		$('#reportTitleDiv').on('hide.bs.dropdown', function () {
		  $('#reportTitle').removeClass('clicked');
			...
			setTimeout(function(){
				$('#titleIcon').removeClass('fas fa-chevron-down').addClass('fas fa-users');
			},75);
		});
		...
		$('.hoverspinner').mouseenter(function(){
			if($('#reportTitle').hasClass('clicked') == false){
				...
				setTimeout(function(){
					$('#titleIcon').removeClass('fas fa-users').addClass('fas fa-chevron-down');
				},75);
			}
		}).mouseleave(function(){
			if($('#reportTitle').hasClass('clicked') == false){
			...
			var doSearch = setTimeout(function(){
				...
				if (values.plant != "") {
					...
					if (values.startDate.trim() != "" && values.endDate.trim() != ""){
						...
						$.ajax({
							type: "get",
							url: "customerutil_CFC.cfc?method=doSearch",
							dataType: "text",
							data: {
								plant: plant,
								customer: customer,
...
//								link.click();
								...
								if (obj.result == 'pass') {
									...
									$('#resultTable > thead').html('<tr><th style="border-top-left-radius: 5px"><div align="right" class="noselect">Delivery Date</div></th><th><div align="right" class="noselect">Plant</div></th><th><div align="right" class="noselect">Customer</div></th><th ><div align="right" class="noselect">Trailer</div></th><th><div align="center" class="noselect">TripNo</div></th><th><div align="left" class="noselect">DR#</div></th><th style="border-top-right-radius: 5px"><div align="left" class="noselect">MSF C-Flute Equivalent</div></th></tr>');
									
									$('#resultTable > tfoot').html('<tr><th></th><th></th><th></th><th></th><th id="uitems"><div align="center">' + total_trips + '</div></th><th></th><th id="total"><div align="left">' + total_shipped_msf + '</div></th></tr>');
									
									var exportTitle = 'CustomerUtil_' + startDate + '__' + endDate;
...
//									}
									...
									var CHART_TYPES = ['CustTotalMSF','CustNumberTrips','CustAvgMSFTrip'];
									...
									for(var i = 0; i < CHART_TYPES.length; i++){
										...
										$.ajax({
											type: "get",
											url: "customerutil_CFC.cfc?method=getChartData",
											dataType: "text",
	...
	//											link.click();
												...
												if (obj2.result == 'pass') {
													...
													if(chart_cust == ''){
														chart_cust = 'All Customers'
													}
													
													var chartSub = chart_plant + ' | ' + chart_cust
